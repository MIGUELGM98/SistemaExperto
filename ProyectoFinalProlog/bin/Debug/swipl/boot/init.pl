...
'$chk_alias_file'(Spec, Exts, Cond, true, CWD, FullFile) :- !,
	...
	term_hash(Cache, Hash),user_input)]).
consult(List) :-
...
'$qlf_up_to_date'(P Level reflects the nesting   in  files compiling other
...
consult(M:X) :-
	...
	load_files(M:Id, [stream(Absolute, DerivedFrom, _)
...
'$check_load_non_module'(File, Module) :-
	...
	throw(error(permission_error(load, source, File),
		    context(load_files/2, Msg))).lFile, QlfFile) :-
	(   exists_file(PlFile)
...
%	eval_license
%
%	Report current_message(error, mixed_directive(Goal))
	).
